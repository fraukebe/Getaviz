...
public class JS2JSON {
	SettingsConfiguration config = SettingsConfiguration.getInstance();
	Log log = LogFactory.getLog(this.getClass());
	DatabaseConnector connector = DatabaseConnector.getInstance();

	public JS2JSON() {
		log.info("JS2JSON has started.");
	...
	private String toJSON(List<Node> list) {
        ...
        log.info("7");
			...
			if ((el.hasLabel(Labels.Value.name()))) {
	...
	private String toMetaDataClass(Node c) {
		...
		StatementResult parent = connector
				.executeRead("MATCH (parent)-[:DECLARES]->(class) WHERE ID(class) = " + c.id() + " RETURN parent");
		if (parent.hasNext()) {
	...
	private String toMetaDataAttribute(Node attribute) {
		...
		String declaredType = "";
		StatementResult parent = connector
				.executeRead("MATCH (parent)-[:DECLARES]->(attribute) WHERE ID(attribute) = " + attribute.id()
						+ " RETURN parent.hash");
		if (parent.hasNext()) {
			belongsTo = parent.single().get("parent.hash").asString();
		}
    Iterable<String> labels = attribute.labels();
    for(String label:labels){
      if(!(label.equals("JavaScript")||label.equals("Value"))){
        declaredType = label;
      }
    }
    // Node type = connector
	...
	private String toMetaDataMethod(Node method) {
		...
		StatementResult parent = connector.executeRead(
				"MATCH (parent)-[:DECLARES|INVOKES]->(method) WHERE ID(method) = " + method.id() + " RETURN parent.hash");
		if (parent.hasNext()) {
			belongsTo = parent.single().get("parent.hash").asString();
		}
	...
	private String toMetaDataEnumValue(Node ev) {
		String belongsTo = "";
		StatementResult parent = connector.executeRead("MATCH (parent)-[:HAS]->(enumValue) WHERE ID(enumValue) = " + ev.id()
			+ " RETURN parent.hash");
		if(parent.hasNext()) {
			belongsTo = parent.single().get("parent.hash").asString();
		}
